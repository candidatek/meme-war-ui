...
import { useAuth as useWalletAuth } from "@/app/hooks/useAuth";
import { useAuth as useTwitterAuth } from "@/lib/auth";
...
export default function WarPage() {
  ...
  const { authState, handleSignIn } = useWalletAuth();
  const { session, isAuthenticated, signIn } = useTwitterAuth();
  ...
  const handleSendMessage = async () => {
    ...
    if (!isAuthenticated) {
      showErrorToast("Please connect your Twitter account first");
      await signIn();
      return;
    }
      ...
      <div className="bg-background/50 rounded-lg p-4 space-y-4">
        <div className="flex justify-between text-sm">
          ...
          <span className="font-medium">
            {formatNumber(token.amountPledged)} {token.ticker}
          </span>
        </div>
        <div className="flex justify-between text-sm">
        ...
        {pledgeAmount && Number(pledgeAmount) > 0 && (
          <div className="bg-muted/30 rounded-lg p-3 space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              Your share:{" "}
              {((Number(pledgeAmount) / token.amountPledged) * 100).toFixed(2)}%
